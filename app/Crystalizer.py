...
class Crystalizer(commands.Cog):
    def __init__(self):
        logging.info("Crystalizer armed B)")

    @commands.command(name="crystalize")
    async def crystalize(self, ctx: commands.Context, *, liberal: discord.User = None):
        if liberal == None:
            if ctx.message.reference != None:
                reply = await ctx.channel.fetch_message(
    ...
    def crystalize(self, avatar_bytes: bytes) -> bytes:
        with (
            ...
            ImageDraw.Draw(mask).ellipse(
                (0, 0, avatar.size[0], avatar.size[1]), fill=255
            )
